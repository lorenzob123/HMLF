...
def get_policy_from_name(policy_type: str, policy_group: str) -> Type[BasePolicy]:
    ...
    if policy_group not in _policy_registry:
        raise KeyError(f"Error: the policy type {policy_group} is not registered!")
    if policy_type not in _policy_registry[policy_group]:
        raise KeyError(
            f"Error: unknown policy type {policy_type},"
            f"the only registed policy type are: {list(_policy_registry[policy_group].keys())}!"
        )
    return _policy_registry[policy_group][policy_type]
...
def register_policy(policy: Type[BasePolicy], policy_type: str, policy_group: str) -> None:
    ...
    if policy_group not in _policy_registry:
        _policy_registry[policy_group] = {}

    if policy_type in _policy_registry[policy_group]:
        # Check if the registered policy is same
        # we try to register. If not so,
        # do not override and complain.
        if _policy_registry[policy_group][policy_type] != policy:
            raise ValueError(f"Error: the name {policy_group} and type {policy_type} is already registered for a different policy, will not override.")
    _policy_registry[policy_group][policy_type] = policy
